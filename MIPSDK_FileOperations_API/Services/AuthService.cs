    ...
    public class AuthService
    {
            ...
            ILogger<AuthService> logger)
        {
            ...
            _defaultScopes = !string.IsNullOrWhiteSpace(_mipOptions.Scopes)
                ...
                : new[] { "https://api.aadrm.com/.default" };
        ...
        public async Task<string> AcquireClientCredentialsTokenAsync(string[]? scopes = null)
        {
            var effectiveScopes = (scopes is { Length: > 0 }) ? scopes : _defaultScopes;
            _logger.LogInformation("Acquiring Client-Credentials token for scopes: {Scopes}", string.Join(", ", effectiveScopes));
            var result = await _cca
                .AcquireTokenForClient(effectiveScopes)
                .ExecuteAsync()
                .ConfigureAwait(false);
            // Log token details
            LogTokenDetails(result.AccessToken, "Client-Credentials Token");
